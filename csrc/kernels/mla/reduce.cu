    ...
    const MlaReduceKernelV1Params params)
{
    ...
    for (int32_t work_idx = blockIdx.x; work_idx < tot_work; work_idx += gridDim.x)
    {
        ...
        const int32_t batch_idx = work_idx / Traits::kNumHeadQ;

        const int32_t reduce_tile_start = params.p_reduce_indptr[batch_idx];
        const int32_t reduce_tile_end = params.p_reduce_indptr[batch_idx + 1];
        ...
        if (reduce_tile_start + 1 < reduce_tile_end)
        {
            ...
            const MlaPartialTileInfo final_loc = [&]()
            {
                if constexpr (Traits::kOmitReduceFinalMap)
                {
                    ...
                    return MlaPartialTileInfo{batch_idx * qo_len, (batch_idx + 1) * qo_len};
                }
                else
                {
                    return params.p_reduce_final_map[batch_idx];
                }
            }();
    ...
    const cudaStream_t&            stream)
{
    ...
    {
        const dim3 grid = dim3(num_cu * Traits::kOccupancy);
